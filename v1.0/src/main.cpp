...
// ─────────────────────────────────────────────────────────────────────────────
//  Training duration — comment/uncomment the line you want
// ─────────────────────────────────────────────────────────────────────────────

// #define TRAINING_DURATION_MS  (24UL * 3600UL * 1000UL)   // 24 h — production
#define TRAINING_DURATION_MS  (60UL *   60UL * 1000UL)   // 60 min — testing
...
#define OLED_REFRESH_WINDOWS   1   // redraw every N completed windows
...
// ─────────────────────────────────────────────────────────────────────────────
//  LED — non-blocking state machine
// ─────────────────────────────────────────────────────────────────────────────
...
enum class LedPattern : uint8_t {
    ...
    SLOW_BLINK, // training  — heartbeat
    FAST_BLINK, // inference — DEVIATION detected
    ON          // solid     — training just finished
};
...
static void oled_show_inference(bool is_baseline, float dist,
    ...
    if (is_baseline) {
        ...
        // Inverted screen for maximum visibility
...
void setup() {
    ...
    // Uncomment to restore a previously trained model instead of always retraining:
    ...
    g_mode = SystemMode::TRAINING;
    g_led_pattern = LedPattern::SLOW_BLINK;
    Serial.printf("[SYSTEM] Starting TRAINING for %lu s.\n",
                  TRAINING_DURATION_MS / 1000UL);
    oled_show_boot_message("Starting\nTRAINING...");
    delay(1500);
}
