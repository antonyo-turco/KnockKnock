...
//  Stores normalised feature vectors collected before k++ kicks in.
// ─────────────────────────────────────────────────────────────────────────────
...
static void welford_update(float &mean, float &M2, uint32_t n, float x) {
    float delta = x - mean;
    mean       += delta / (float)n;
    M2         += delta * (x - mean);   // correct Welford form (uses updated mean)
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  K-means++ seeding  (furthest-point / greedy variant — deterministic)
//
//  Classic k++ picks each new centroid with probability ∝ D²(x).
//  For an embedded system without a reliable RNG we use the deterministic
//  equivalent: always pick the sample that MAXIMISES the min-distance to the
//  already-chosen centroids.  This spreads centroids at least as aggressively
//  as the probabilistic version and is fully reproducible.
//
//  Complexity: O(SEED_BUFFER_SIZE × KMEANS_K²) — with K=4 and buffer=100
//  that is ~1600 distance evaluations, negligible.
// ─────────────────────────────────────────────────────────────────────────────

static void kmeans_pp_seed(KMeansModel &model) {
    // C0 — take the first buffered sample as the starting centroid
    memcpy(model.centroids[0], s_seed_buf[0], sizeof(float) * FEATURE_DIM);
    model.centroid_counts[0] = 1;

    for (int k = 1; k < KMEANS_K; ++k) {
        // For every buffered sample, compute its distance to the nearest
        // centroid chosen so far.  Pick the sample with the largest such
        // distance — it is the most "underrepresented" point in the buffer.
    ...
    Serial.printf("[TRAINING] k++ seeding done on %d samples — %d centroids placed.\n",
...
void training_init(KMeansModel &model) {
    ...
    for (int d = 0; d < FEATURE_DIM; ++d) {
        model.norm_std[d] = 1.0f;   // prevent divide-by-zero before data arrives
    }
    // Reset seeding state
    s_seed_count   = 0;
...
void training_update(KMeansModel &model, const InferenceFeatures &features) {
    ...
    // ── 1. Update normalisation stats (always, even during seeding) ──────────
    model.norm_n++;
    for (int d = 0; d < FEATURE_DIM; ++d) {
    ...
    // ── 2. Normalise current sample ──────────────────────────────────────────
    float norm_fv[FEATURE_DIM];
    normalise(model, raw, norm_fv);

    // ── 3. Fill seeding buffer until it is full, then run k++ ────────────────
    if (!s_seeding_done) {
        ...
        // Do not update centroids yet — wait for seeding to finish
        return;
    }

    // ── 4. Online centroid update  (arithmetic running mean) ─────────────────
    //       c ← c + (x − c) / n   — equivalent to the batch mean, no LR needed
    ...
    // ── 5. Track distance stats for threshold computation ────────────────────
    float dist = euclidean_dist(norm_fv, model.centroids[k], FEATURE_DIM);

    // Welford mean + variance of distances
    model.dist_n[k]++;
    welford_update(model.dist_mean[k], model.dist_M2[k], model.dist_n[k], dist);

    // Track maximum distance seen — used for the max-based branch of threshold
...
void training_finalize(KMeansModel &model) {
    ...
    //   thr_stat = mean_dist + SIGMA_MULT * sigma_dist
    //   thr_max  = dist_max  * MAX_DIST_MARGIN
    //   threshold = max(thr_stat, thr_max, MIN_THRESHOLD)
    //
    // thr_stat captures the typical spread of normal samples.
    // thr_max  guarantees that every sample seen during training is classified
    //          as baseline — including legitimate spikes (trucks, wind, etc.)
    //          that would otherwise become false positives in inference.
...
bool training_is_baseline(const KMeansModel &model,
    ...
    if (!model.finalised) {
        ...
        return true;   // safe default — don't alarm while untrained
    }
...
void training_print_model(const KMeansModel &model) {
    Serial.println(F("\n╔══════════════════════════════════════════════╗"));
    Serial.println(F("║     K-Means++ Model  (14-dim)                ║"));
    Serial.println(F("╚══════════════════════════════════════════════╝"));
