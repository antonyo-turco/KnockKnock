...
#define SEND_INTERVAL_MS   5000  // 1 secondi
...
bool send_bulk_data(){
    if (WiFi.status()!=WL_CONNECTED){ Serial.println("✗ WiFi not connected!"); return false; }
    if (buffer_index==0){ Serial.println("Buffer empty, nothing to send"); return true; }
    Serial.printf("\n=== Sending batch #%lu ===\n", (unsigned long)batch_seq);
    Serial.printf("Samples: %d\n", buffer_index);
    static char json_buffer[JSON_BUFFER_SIZE];
    char* pos=json_buffer; size_t remaining=JSON_BUFFER_SIZE;
    int n = snprintf(pos, remaining, "{\"device_id\":\"esp32-heltec-v4\",\"batch_seq\":%lu,\"sampling_rate_hz\":%d,\"send_interval_ms\":%d,\"measurements\":[", (unsigned long)batch_seq, SAMPLING_RATE_HZ, SEND_INTERVAL_MS);
    if (n<=0 || (size_t)n>=remaining){ Serial.println("✗ JSON header overflow"); return false; }
    pos+=n; remaining-=(size_t)n;
    for (int i=0;i<buffer_index;i++){
        n = snprintf(pos, remaining, "%s{\"x\":%d,\"y\":%d,\"z\":%d}", (i>0)?",":"", buffer[i].x, buffer[i].y, buffer[i].z);
        if (n<=0 || (size_t)n>=remaining){ Serial.println("✗ JSON body overflow"); return false; }
        pos+=n; remaining-=(size_t)n;
    }
    n = snprintf(pos, remaining, "]}"); if (n<=0 || (size_t)n>=remaining){ Serial.println("✗ JSON footer overflow"); return false; }
    ...
    if (httpResponseCode==201){ Serial.printf("✓ Server OK: %d measurements received\n", buffer_index); String response = http.getString(); Serial.println("Response: "+response); buffer_index=0; batch_seq++; last_send=millis(); http.end(); return true; }
    Serial.printf("✗ Server error: %d\n", httpResponseCode); String response = http.getString(); Serial.println("Response: "+response); http.end(); return false;
}
...
void setup(){
    ...
    Serial.println("\n=== ADXL362 + Bulk WiFi (metadata enabled) ==="); Serial.printf("Sampling: %d Hz\n", SAMPLING_RATE_HZ); Serial.printf("Send interval: %d ms\n", SEND_INTERVAL_MS);
    ...
    delay(100);
    last_send = millis(); Serial.println("Ready to accumulate data...\n");
}
...
void loop(){
    ...
    if (buffer_index < BUFFER_SIZE) { buffer[buffer_index].x = x_raw; buffer[buffer_index].y = y_raw; buffer[buffer_index].z = z_raw; buffer_index++; }
    if (sending_enabled && (millis() - last_send >= SEND_INTERVAL_MS)) { send_bulk_data(); }
