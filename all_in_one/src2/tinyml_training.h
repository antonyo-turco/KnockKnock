...
static constexpr float MAX_DIST_MARGIN   = 1.1f;   // 10% above worst training sample

// Absolute floor — avoids hair-trigger alarms on perfectly silent clusters
static constexpr float MIN_THRESHOLD     = 0.30f;

// K-means++ seeding: number of samples collected before choosing centroids.
// 100 windows × ~2 s/window = ~3 min of warm-up.
// Buffer lives in static RAM (not NVS): 100 × 14 × 4 B = 5.6 kB.
static constexpr int   SEED_BUFFER_SIZE  = 100;
