...
static bool read_sample(Sample &sample) {
    ...
    sample.x = static_cast<int16_t>(x_raw * SIGNAL_GAIN);
    sample.y = static_cast<int16_t>(y_raw * SIGNAL_GAIN);
    sample.z = static_cast<int16_t>(z_raw * SIGNAL_GAIN);
    return true;
}
...
static void process_window() {
    ...
    Serial.printf(
        "Window #%lu | impact=%.4f | x_p99=%.2f | y_p99=%.2f | z_p99=%.2f | m_p99=%.2f | %s\n",
        static_cast<unsigned long>(window_seq),
        features.impact_score,
        features.x_p99,
        features.y_p99,
        features.z_p99,
