...
enum class LedPattern : uint8_t {
    OFF,         // inference baseline
    PULSE,       // exploring — gentle double-blink every 3 s
    SLOW_BLINK,  // training  — 1 Hz
    FAST_BLINK,  // inference deviation
    ON           // brief solid at phase transitions
};
...
static void led_update() {
    ...
    switch (g_led_pattern) {
        ...
        case LedPattern::PULSE:
            // Double-blink pattern: on-off-on-off-long pause (period ~3 s)
            // Steps: 0=on(100ms) 1=off(100ms) 2=on(100ms) 3=off(2700ms)
            {
                static const uint16_t durations[] = {100, 100, 100, 2700};
                static const bool     states[]    = {true, false, true, false};
                if (now - g_led_last_ms >= durations[g_led_step]) {
                    g_led_step = (g_led_step + 1) % 4;
                    g_led_state = states[g_led_step];
                    digitalWrite(ALERT_LED_PIN, g_led_state ? HIGH : LOW);
                    g_led_last_ms = now;
                }
            }
...
static void oled_show_exploring() {
    ...
    oled.setTextSize(2);
    oled.setCursor(0, 0);
    oled.print(F("EXPLORE"));
    oled.setTextSize(1);
    oled.setCursor(0, 18);
    oled.printf("%u%%  rem:%s", pct, tbuf);
    oled_progress_bar(0, 30, 128, 10, pct);
    oled.setCursor(0, 44);
    oled.printf("novel:%u%%  n=%lu",
...
static void init_sensor() {
    ...
    adxl362_write_reg(ADXL362_REG_FILTER_CTL, 0x13);  // ±2g, 100 Hz
    adxl362_write_reg(ADXL362_REG_POWER_CTL,  0x02);  // measurement
    delay(100);
}
...
static void transition_to_training() {
    ...
    if (!ok) {
        Serial.println("[SYSTEM] WARNING: novelty buffer sparse — model may need longer exploring.");
    }

    g_phase_start_ms = millis();
    g_mode = SystemMode::TRAINING;
    g_led_pattern = LedPattern::SLOW_BLINK;
    Serial.printf("[SYSTEM] EXPLORING complete -> TRAINING for %lu s.\n",
                  TRAINING_DURATION_MS / 1000UL);
    oled_show_message("TRAINING", "started...");
    delay(1500);
}
...
void setup() {
    ...
    Serial.printf("Window: %d samples @ %.0f Hz = %.2f s\n",
                  SAMPLE_COUNT, SAMPLING_RATE_HZ, SAMPLE_COUNT / SAMPLING_RATE_HZ);
    Serial.printf("Exploring: %lu s | Training: %lu s\n",
                  EXPLORING_DURATION_MS / 1000UL, TRAINING_DURATION_MS / 1000UL);
    ...
    // Try to load a previously finalised model
    // if (training_load(g_model)) {
    //     g_mode = SystemMode::INFERENCE;
    //     g_led_pattern = LedPattern::OFF;
    //     Serial.println("[SYSTEM] Model loaded -> INFERENCE mode.");
    //     oled_show_message("Model loaded", "INFERENCE mode");
    //     delay(1500);
    ...
    //     oled_show_message("No model found", "EXPLORING...");
    //     delay(1500);
    // }
