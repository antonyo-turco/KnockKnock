...
adxl362_handle_t sensor_init(float frequency_hz) {
    ...
    /* ADXL362 ODR register values; 200 Hz is the target for 512-sample FFT at
     * 0.39 Hz/bin resolution. Lower rates are retained for bench testing. */
    switch ((int)frequency_hz) {
        ...
        default:
            ESP_LOGW(TAG, "Unsupported ODR %d Hz — defaulting to 100 Hz", (int)frequency_hz);
    ...
    /* Allow the sensor to settle and take a first batch of samples before
     * the caller starts reading, so the activity interrupt is clean. */
    vTaskDelay(pdMS_TO_TICKS(300));

    uint8_t status = 0;
    adxl362_get_status(sensor, &status);
    if (status & (1 << 4)) {
        /* Activity flag set during settle — clear it with a second read */
