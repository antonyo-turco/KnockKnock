/**
 * @file main.cpp
 * @brief ADXL362 + WiFi bulk sender con metadati per feature extraction lato server
 */
...
/* =========================================================
 *  Heltec V4 (ESP32-S3) Pin Configuration
 * ========================================================= */
#define PIN_MOSI         1
#define PIN_MISO         2
#define PIN_SCK          3
...
/* =========================================================
 *  Campionamento e batch
 * ========================================================= */
#define SAMPLING_RATE_HZ   100
#define SAMPLE_PERIOD_MS   10
#define BUFFER_SIZE        6000
#define SEND_INTERVAL_MS   10000  // 10 secondi
...
bool send_bulk_data()
{
    ...
    int n = snprintf(
        pos,
        remaining,
        "{\"device_id\":\"esp32-heltec-v4\",\"batch_seq\":%lu,\"sampling_rate_hz\":%d,\"send_interval_ms\":%d,\"measurements\":[",
        (unsigned long)batch_seq,
        SAMPLING_RATE_HZ,
        SEND_INTERVAL_MS
    );
...
void setup()
{
    ...
    Serial.println("\n=== ADXL362 + Bulk WiFi (metadata enabled) ===");
...
void loop()
{
    ...
    if (buffer_index < BUFFER_SIZE) {
        buffer[buffer_index].x = x_raw;
        buffer[buffer_index].y = y_raw;
        buffer[buffer_index].z = z_raw;
        buffer_index++;
    }
